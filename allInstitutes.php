...
if(isset($_REQUEST['search'])){
    ...
    @header("location:allInstitutes.php?searchQuery=".$search);
}
...
if(isset($_REQUEST['filter'])){
    ...
    $location = "allInstitutes.php?filtering=1&";
    if(!empty($subject)){
...
<section class="bg-[#f1f1f1] h-screen pt-12"> 
    <!-- <button id="openFilter" class="bg-white shadow-lg border-t-2 border-black/20 fixed flex items-center justify-between h-8 w-32 px-4 rounded-md text-lg z-[1]">Filter 
    ...
    <main class="justify-center items-center w-full gap-8 flex flex-wrap pt-10">
         <?php
            $teacherQuery = "SELECT * FROM `teacher-table` WHERE `activity` = 1 AND `role` = 'Institute'";
            ...
            while($rowarr = mysqli_fetch_array($res))
            {
                ...
                $profilequery = "SELECT profileImagePath FROM `profile-image-table` WHERE `id` = '$randTeacherId'";
                $profilePath = mysqli_fetch_array(mysqli_query($connection,$profilequery))['profileImagePath'];
        ?>
        <!-- cards -->
                <section class=" relative w-96  rounded-2xl shadow-lg py-8 flex flex-col gap-4">
                    <!-- profile image area with absolute property -->
                    <div class="px-4 h-20 w-full p-1 rounded-full flex ">
    ...
    <section class="bg-[#f1f1f1] w-full relative py-8">
        <ul  class="sticky flex gap-6 justify-center" >
            <?php 
                $totalRows = "SELECT * FROM `teacher-table` WHERE `activity` = 1 AND `role` = 'Institute'";
            ...
            ?>
            <a class="h-8 w-8" href="allInstitutes.php?page=<?php echo $i;?>">
                <li class="h-full w-full bg-blue-500 text-white text-center leading-[2rem] rounded-md">
                    <?php echo $i;?></li>
            </a>
            <?php } ?>
        </ul>
